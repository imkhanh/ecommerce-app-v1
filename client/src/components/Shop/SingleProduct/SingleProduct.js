...
import { updateQuantity, addToCart, cartList, nextSlide, prevSlide } from './Functions';
import { BsArrowLeft, BsHeartFill, BsChevronLeft, BsChevronRight } from 'react-icons/bs';
					...
					<div className="grid grid-cols-2 lg:grid-cols-1 md:grid-cols-2 gap-3">
						{product.quantity !== 0 ? (
							inCart && inCart.includes(product._id) ? (
								...
								<button
									onClick={() => addToCart(product._id, qty, product.price, setQty, dispatch, fetchSingleProduct)}
