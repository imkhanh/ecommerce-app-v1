...
const Register = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const res = await register({ fullName: data.fullName, userName: data.userName, email: data.email, password: data.password });
			if (res.data.success) {
				setData({ ...data, fullName: '', userName: '', email: '', password: '', error: false, success: res.data.success, loading: false });
			} else {
				setData({ ...data, email: '', password: '', error: res.data.error, success: false, loading: false });
			}
		} catch (error) {
			console.log(error);
		}
			...
			<form onSubmit={handleSubmit}>
				...
				<div className="mb-4">
					<p className="p-3 text-center text-xs text-black/40 font-light">
						By creating an account, you agree to .K's <span className="underline underline-offset-2">Privacy Policy</span> and
