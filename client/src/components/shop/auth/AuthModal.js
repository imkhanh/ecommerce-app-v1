			...
			<div className={`${state.authModal ? '' : 'hidden'} fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2`}>
				<div className="p-8 w-[384px] h-auto rounded-sm bg-white shadow-lg space-y-8">
					<div className="text-center">
						<h1 className="mb-4 text-sm uppercase font-bold">Blancos</h1>
						<p className="text-xl uppercase font-medium leading-6">{login ? 'Your account for everything shop' : 'Become a member shop'}</p>
					</div>
					...
					<div className="text-center">
						<p onClick={() => setLogin(!login)} className="text-xs font-light text-black/60">
							...
							<span className="ml-1 text-black font-normal underline underline-offset-2 cursor-pointer select-none">
