...
const customizeController = {
	getAllDocuments: async (req, res) => {
		try {
			...
			if (users && categories && products && orders) return res.json({ users, products, categories, orders });
		} catch (error) {
			console.log(error);
		}
	},
	...
	deleteImage: async (req, res) => {
		try {
			...
			const filePath = path.join(__dirname, `../client/build/uploads/customizes/${image.slideImage}`);
